...
while True:
    ...
    if gamestate.menu_state in [melee.Menu.IN_GAME, melee.Menu.SUDDEN_DEATH]:
        ...
        if args.connect_code != "":
            discovered_port = melee.gamestate.port_detector(gamestate, melee.Character.DOC, costume)
        if discovered_port > 0:
            # NOTE: This is where your AI does all of its stuff!
            # This line will get hit once per frame, so here is where you read
            #   in the gamestate and decide what buttons to push on the controller
            melee.techskill.upsmashes(ai_state=gamestate.players[discovered_port], controller=controller)
        else:
